...
async function sendErrorLog(client, error, type) {
  try {
    ...
    if (
      ...
      return client.logger.error("ERR_LOG", error);
    }

    const { logChannelId } = require("../../config.json");
    if (!logChannelId) return client.logger.error("ERR_LOG", error?.stack || `${error}`);

    const channel =
      client.channels.cache.get(logChannelId) || (await client.channels.fetch(logChannelId));
