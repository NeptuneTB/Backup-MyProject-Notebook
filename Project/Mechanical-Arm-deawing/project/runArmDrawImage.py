...
from myCannyEdge import non_max_suppression, hysteresis, threshold
...
#==============First Half=========================
for i in range(posPix.shape[1]):
    print(x[i], ', ', y[i], ' = ', newSize_image[x[i], y[i]])
    Step = int(mstep[y[i], x[i]])
    ...
    controller.move(1, int(m1[y[i], x[i]]), 1000)
    time.sleep(dl)
    controller.move(4, int(m4[y[i], x[i]]), 1000)
    time.sleep(dl)
    controller.move(3, int(m3[y[i], x[i]]), 1000)
    time.sleep(dl)
    # print('s3')
    ...
    print(res)
    time.sleep(1)
    controller.move(2, int(m2[y[i], x[i]]), 1000)
    time.sleep(dl)
...
for i in range(posPix1.shape[1]):
    print(x1[i], ', ', y1[i], ' = ', newSize_image[x1[i], y1[i]])
